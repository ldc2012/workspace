...
static pthread_mutex_t mutex_logdata = PTHREAD_MUTEX_INITIALIZER;
...
int write_logdata(const char *type, const char *format,...)
{
	...
	pthread_mutex_lock(&mutex_logdata);
	FILE *stream = fopen(pathname_logdata, "a");
        if (!stream) {
		va_start(ap, format);
		vfprintf(stderr, format, ap);	// stderr not flush line
		va_end(ap);
	...
	pthread_mutex_unlock(&mutex_logdata);

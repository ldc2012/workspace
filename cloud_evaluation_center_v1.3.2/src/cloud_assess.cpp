...
int cmp(float **p, int * remain_number, int sort)
{
	if(sort ==1)
	//按从大到小顺序排列二维数组
...
int assess_char(char * buff, const rule ruletype, float **p,const int remain_number)
{
//	printf("assess_char_start");
	char top[50],body[50];

	memset(top, 0 , sizeof(top));
	memset(buff, 0, sizeof(buff));
	strcpy(buff, "#");
	sprintf(top, "%d=%d;", ruletype, remain_number);
	strcat(buff, top);
	for(int i = 0; i < remain_number ; i++)
	{
		memset(body, 0, sizeof(body));
		sprintf(body, "%d=%.2f;", (int)p[i][0], p[i][1]);
		strcat(buff, body);
	}
...
void iass_info_flush(float **p, const int remain_number)
{
	for(int i = 0; i < remain_number; ++i){
		p[i][1] = 0 ;
	}
...
//淘汰算法
int eliminate(float **p, int * remain_number, float dif)
{
	float last;
...
//阀值算法
int valve_value(float **p, int * remain_number, float dif)
{
	//判断阀值
	for(int j = 1; j < *remain_number; j++){
		if(p[j][1] >=  dif){
...
void assess(MYSQL *my)
{
	...
	char sql[2048], time_str[64], *assess_str, *tmp_str;
	time_t datatime;
	float **iaas_info;
...
*/	
	assess_str = (char *)malloc(10 * sizeof(char) * cloud_number * (g_confvalue.rule_num+1));
	if(NULL == assess_str)
		printf("assess_str null\n");
	memset(assess_str, 0, sizeof(assess_str));
	printf("memsize:%d\n",(10 * sizeof(char) * cloud_number * (g_confvalue.rule_num+1)));
	printf("assess_str:%p \n", assess_str);
/*	if(NULL != tmp_str){
		free(tmp_str);
		tmp_str = NULL;
...
*/	
	tmp_str = (char *)malloc(20 * sizeof(char) * cloud_number);
	printf("tmp_str:%p \n", tmp_str);
	...
	for(int i = 0; i < g_confvalue.rule_num; i++){
		...
		switch(rule_type)
		{
			case CPU_OVERLOAD:
					//printf("CPU_OVERLOAD \n");
				iass_info_flush(iaas_info, remain_number);
				//取所有云环境CPU平均值
				if(-1 != get_avg(my, iaas_info, remain_number, rate[0],datatime)){
				//排序
					cmp(iaas_info, &remain_number, 0);
					printf("cmp finish\n");
				//字符串拼接
					assess_char(tmp_str, rule_type, iaas_info, remain_number);
					strcat(assess_str, tmp_str);
					printf("assess_str:%s \n", assess_str);
				//阀值判断
					valve_value(iaas_info, &remain_number, g_confvalue.cpu_avg_2);
				}
				else 
			...
			case MEM_OVERLOAD:
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//阀值判断
					valve_value(iaas_info, &remain_number, g_confvalue.mem_avg_2);
				}
				else 
...
//					printf("IO_OVERLOAD \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//阀值判断
					valve_value(iaas_info, &remain_number, g_confvalue.io_avg_2);
				}
				else 
...
//					printf("CPU_AGE \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.avg_dif.cpu);
				}
				else 
					logerr("get AVG(cpu_use_rate) of iaas filed.");
				break;
...
//					printf("MEM_AGE \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.avg_dif.mem);
				}
				else 
					logerr("get AVG(memory_use_rate) of iaas filed.");
				break;
...
//					printf("IO_AGE \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.avg_dif.io);
				}
				else 
					logerr("get AVG(io_use_rate) of iaas filed.");
				break;
...
//					printf("CPU_ASSIGN \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.assign_dif.cpu);
				}
				else 
...
//					printf("MEM_ASSIGN \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.assign_dif.mem);
				}
				else 
...
//					printf("IO_ASSIGN \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.assign_dif.io);
				}
				else 
...
//					printf("DOMAIN_SELL \n");
				...
				printf("assess_str:%s \n", assess_str);
				//淘汰算法
				eliminate(iaas_info, &remain_number,g_confvalue.domain_sell_dif);
				break;
			case CPU_AVAILABLE:
				break;
			case MEM_AVAILABLE:
...
//					printf("CPU_MAX \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number, g_confvalue.max_dif.cpu);
				}
				else 
...
//					printf("MEM_MAX \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.max_dif.mem);
				}
				else 
...
//					printf("IO_MAX \n");
				...
				//字符串拼接
					...
					printf("assess_str:%s \n", assess_str);
				//淘汰算法
					eliminate(iaas_info, &remain_number,g_confvalue.max_dif.io);	
				}
				else 
	...
	//字符串拼接
	assess_char(tmp_str, rule_type, iaas_info, 1);
	strcat(assess_str, tmp_str);
	printf("assess_str:%s \n", assess_str);
	printf("tmp_str:%p \n", tmp_str);
	printf("assess_str:%p \n", assess_str);
	//结果插入数据库
	printf("1\n");
	sprintf(sql,"INSERT INTO iaas_performance_evaluation_data (iaas_id, data_time, evaluation_data) VALUES ('%f', '%ld', '%s')", iaas_info[0][0], datatime, assess_str);
